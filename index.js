...
client.connect().then(() => {console.log("Successfully connected to MongoDB!");
    ...
    const PaymentCollections = client
      .db("ContestCreationDb")
      .collection("payments");
    const SubmissionCollections = client
      .db("ContestCreationDb")
...
// get all contest for search
    ...
    app.patch("/users/:userEmail", async (req, res) => {
      ...
      const result = await UserCollections.updateOne({ email: userEmail }, { $set: updateFields });
      res.send(result);
    });
    ...
    app.get("/pending-rejected-contests", async (req, res) => {
      const result = await ContestCollections.find({
...
app.get("/submission-contest/:userEmail/:contestId", async (req, res) => {
  ...
  res.send(submissions);
    ...
    app.patch("/make-winner/:submissionId", async (req, res) => {
      ...
      const submission = await SubmissionCollections.findOne({
      ...
      const existingWinner = await SubmissionCollections.findOne({
        contestId: submission.contestId,
        isWinner: true,
      });
      if (existingWinner) {
        return res.status(400).send({
          message: "A winner has already been chosen for this contest",
        });
      }
    ...
    app.get("/user-points/:userEmail", async (req, res) => {
      const userEmail = req.params.userEmail;
      const winningSubmissions = await SubmissionCollections.find({
        userEmail,
        isWinner: true,
      }).toArray();
